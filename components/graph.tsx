...
export function TransactionsGraph({address, graphData, setGraphData }: TransactionsGraphProps) {
    ...
    useEffect(() => {
        if (data) {
            ...
            const graph = buildGraphDataFromTransactions(aggregatedtransactions, address, provider);
            setGraphData(graph);
        }
        ...
        {loading ? (
                ...
                // Render your graph or other components here
                    ...
                    nodeThreeObject={(node: any) => {
                        const sprite =  node.starknetId ? new SpriteText(node.starknetId) : new SpriteText(node.id.substring(0, 12));
                        sprite.color = 'black';
                        sprite.textHeight = 1.5;
